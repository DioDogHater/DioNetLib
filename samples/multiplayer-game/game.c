...
void load_assets(){
	player_texture = load_texture("../samples/multiplayer-game/assets/player.png");
	if(player_texture)
		printf("assets/player.png loaded!\n");
	terminal_font = TTF_OpenFont("../samples/multiplayer-game/assets/terminal.ttf",25);
	if(terminal_font)
		printf("assets/terminal.ttf loaded!\n");
}

void free_assets(){
	if(player_texture)
		SDL_DestroyTexture(player_texture);
	if(fps_counter_texture)
...
void handle_packet(int data_size){
	int buffer_pos=0;
	// Parse through all packets
	while(buffer_pos < data_size){
		switch(buffer[buffer_pos]){
			// If a player quit, remove them and reset their state
			case (char)PKG_PLAYER_QUIT:{
				...
				buffer_pos++;
				break;
			// If a player sends an update, update their state
...
int main(int argc, char* argv[]){
	...
	for(int i=0; i < MAX_CLIENTS; i++){
		...
		player_positions[i]=(SDL_Point){0,0};
	}
	...
	while(true){
		...
		if(NOW_TICK-LAST_TICK >= FPS_CAP){
			...
			char fps_counter_string[25];
			sprintf(fps_counter_string,"FPS: %.1f",FPS);
			if(fps_counter_texture)
				SDL_DestroyTexture(fps_counter_texture);
